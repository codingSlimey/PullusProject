  ...
  return (
    ...
    <div className='flex items-center text-primary ' >
        <div className='text-start' >
            ...
            <p className='font-normal text-sm'>This is so far going to be the best offer you may ever get</p>
        </div>
        <div>
            <img src={bird} className='' alt='bird' />
        </div>
            ...
            icon.map((item, index) => {
                return (
                    <div key={index} className='flex flex-col items-center justify-center' >
                        <img className=' w-16 h-16 p-3 rounded-full bg-grey' src={item.src} alt={item.alt} />
                        <p>{item.text}</p>
                    </div>
                )
            })
    ...
    <div className='flex gap-3 hide-scrollbars overflow-x-auto'>
        <h1 className='border-2 border-primary px-3 py-2 text-primary font-medium hover:bg-primary hover:text-white rounded-3xl' >All</h1>
        {
          icon.map((item, index) => {
            return (
            <div className='border-2 border-primary py-2 px-3 rounded-3xl text-primary font-medium hover:bg-primary hover:text-white ' >
                {item.text}
            </div>
            )
        })  
